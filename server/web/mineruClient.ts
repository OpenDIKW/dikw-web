...
const INPUT_CODES = new Set(["-60002", "-60005", "-60006", "-30001", "-30002", "-30003"]);
...
export type MineruFetch = (url: string | URL, init?: RequestInit) => Promise<Response>;
...
export class MineruClient {
  ...
  async submit(params: SubmitParams): Promise<SubmissionHandle> {
    ...
    const batchId = typeof data?.batch_id === "string" && data.batch_id ? data.batch_id : null;
  ...
  async upload(uploadUrl: string, body: Uint8Array | Blob): Promise<void> {
    ...
    if (response.status >= 400) {
      throw new MineruClientError("mineru_api", `MinerU upload failed: HTTP ${response.status}`);
    }
  }
    ...
    while (true) {
      ...
      if (Array.isArray(extract)) {
        ...
        if (head !== undefined && !isRecord(head)) {
          throw new MineruClientError(
            "mineru_api",
            scrub(`MinerU poll extract_result[0] not a dict: ${JSON.stringify(body)}`, this.token),
          );
        }
        ...
        throw new MineruClientError(
          "mineru_api",
          scrub(`MinerU poll extract_result not a list: ${JSON.stringify(body)}`, this.token),
        );
      }
      ...
      if (rawState !== undefined && typeof rawState !== "string") {
        throw new MineruClientError(
          "mineru_api",
          scrub(`MinerU poll state is not a string: ${JSON.stringify(body)}`, this.token),
        );
      }
      ...
      if (state === STATE_DONE) {
        ...
        if (typeof fullZipUrl !== "string" || !fullZipUrl) {
          throw new MineruClientError(
            "mineru_api",
            scrub(`MinerU task done but full_zip_url missing: ${JSON.stringify(body)}`, this.token),
          );
        }
        return fullZipUrl;
      }
  ...
  private raiseForCode(code: unknown, msg: unknown, context: string): never {
