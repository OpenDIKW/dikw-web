...
4. **Final pass.** Run `/simplify` (cleanup) or `/code-review xhigh` (depth-first audit) and resolve anything they surface.
...
7. **Create the PR.** Branch with a descriptive name, commit with `<type>(<scope>): <subject>` matching the project's existing convention (see recent `git log`), push, then `gh pr create`. CI auto-runs typecheck + coverage + build + e2e + Trivy. (`/ship` expects gstack scaffolding — `VERSION` + `CHANGELOG.md` — that this repo doesn't carry; use `gh pr create` directly unless that changes.)
