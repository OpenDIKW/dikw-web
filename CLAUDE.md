...
- `serverUrl` and `token` live in `sessionStorage`; `locale`, `theme`, the app-sidebar collapsed state (`sidebarCollapsed`), and the Base directory-panel width (`wikiSidebarWidth`) live in `localStorage`. Keys are namespaced `dikw-web.*`.
