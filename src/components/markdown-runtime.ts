// Shared markdown rendering runtime for the Base reader. Extracted from
// MarkdownView so both the single-column reader and the bilingual dual-column
// view render through the exact same markdown-it instance, sanitizer, and
// post-render hydration (mermaid / ECharts / authenticated images).
//
// `renderMarkdown` renders a whole document; `renderMarkdownBlockHtml` renders
// one block at a time while sharing a heading-slug env across blocks, so a
// bilingual column produces the same `id` slugs a whole-document render would.
// `runMarkdownEffects` / `useMarkdownEffects` hydrate the rendered HTML.
...
/** Render a whole markdown document to sanitized HTML. */
export function renderMarkdown(body: string, ctx: MarkdownContext): string {
  return renderWithEnv(body, ctx, { dikwContext: ctx });
}

/**
 * Render a single markdown block, threading a shared `env` across calls so
 * heading-slug deduplication (and thus the `id` attributes anchor links target)
 * stays consistent with a whole-document render. Callers create one `env`
 * object and pass it for every block in document order.
 */
export function renderMarkdownBlockHtml(
  ...
  env.dikwContext = ctx;
  return renderWithEnv(body, ctx, env);
}

function renderWithEnv(body: string, ctx: MarkdownContext, env: Record<string, unknown>): string {
  ...
  const html = markdown.render(markdownBody, env);
...
/**
 * Hydrate rendered markdown HTML inside `root`: render mermaid diagrams, mount
 * ECharts charts, and swap authenticated image placeholders for blob URLs.
 * Returns a disposer that cancels in-flight work and frees chart instances /
 * blob URLs. Pure DOM — no React — so it serves both the single-column reader
 * and the bilingual dual-column view.
 */
export function runMarkdownEffects(
  root: HTMLElement,
  { assetToken }: { assetToken: string },
): () => void {
  let cancelled = false;
  void renderMermaidDiagrams(root, () => cancelled);
  const disposeCharts = renderMarkdownCharts(root);
  const disposeImages = hydrateAuthenticatedImages(root, assetToken);
  return () => {
    cancelled = true;
    disposeCharts();
    disposeImages();
...
/**
 * React wrapper over {@link runMarkdownEffects}. Re-runs whenever `renderKey`
 * (the rendered HTML, or any value that changes when the DOM content does) or
 * `assetToken` changes, mirroring MarkdownView's original `[html, assetToken]`
 * effect dependencies.
 */
export function useMarkdownEffects<T extends HTMLElement>(
  ref: RefObject<T | null>,
  { renderKey, assetToken }: { renderKey: string; assetToken: string },
): void {
  useEffect(() => {
    ...
    return runMarkdownEffects(root, { assetToken });
    // `renderKey` is an intentional re-run trigger: the DOM is replaced via
    // dangerouslySetInnerHTML by the caller, so effects must re-hydrate the
    // fresh nodes even though renderKey isn't read inside the callback.
  }, [ref, renderKey, assetToken]);
}
