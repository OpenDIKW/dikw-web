...
describe("pipeline persistence", () => {
  ...
  it("save refuses to overwrite an existing coreUrl tag with a different current core", () => {
    ...
    // against B.
    savePipelineState({ stage: "ingest", ingestTaskId: "t-1", coreUrl: CORE }, "http://other.core");
  ...
  it("clears storage on idle save", () => {
    sessionStorage.setItem(
      PIPELINE_STORAGE_KEY,
      JSON.stringify({ stage: "ingest", ingestTaskId: "t-x" }),
    );
  ...
  it("treats a task stage without its task id as unrecoverable", () => {
    sessionStorage.setItem(PIPELINE_STORAGE_KEY, JSON.stringify({ stage: "synth", coreUrl: CORE }));
    expect(loadPipelineState(CORE)).toEqual({ stage: "idle" });
  });
...
describe("activeTaskId / isTaskStage", () => {
  it("returns the running task id for each task stage", () => {
    expect(activeTaskId({ stage: "ingest", ingestTaskId: "i-1" })).toBe("i-1");
    expect(activeTaskId({ stage: "synth", synthTaskId: "s-1" })).toBe("s-1");
    expect(activeTaskId({ stage: "lint-propose", lintProposeTaskId: "lp-1" })).toBe("lp-1");
    expect(activeTaskId({ stage: "lint-apply", lintApplyTaskId: "la-1" })).toBe("la-1");
  });
